...
public:
  ...
  void compute(const double *x, int x_length,
...
private:
  ...
  void generalBody(double current_f0, double current_position,
		   const ForwardRealFFT &forward_real_fft,
		   const InverseRealFFT &inverse_real_fft,
		   double *spectral_envelope);

  void getWindowedWaveform(double current_f0, double currnet_position,
			   const ForwardRealFFT &forward_real_fft);
  ...
  void smoothingWithRecovery(double f0,
			     const ForwardRealFFT &forward_real_fft,
			     const InverseRealFFT &inverse_real_fft,
			     double *spectral_envelope);
