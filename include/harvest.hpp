...
private:
  ...
  int generalBodySub(const double *boundary_f0_list, int number_of_channels,
		     const fft_complex *y_spectrum, int fft_size, int max_candidates);

  void prepareFFTs();
  void destroyFFTs();

  void getWaveformAndSpectrum(int fft_size, int decimation_ratio, fft_complex *y_spectrum);

  void getRawF0Candidates(const double *boundary_f0_list, int number_of_bands,
			  const fft_complex *y_spectrum, int fft_size,
			  double **raw_f0_candidates);
  
  void getFilteredSignal(double boundary_f0, int fft_size, const fft_complex *y_spectrum,
			 double *filtered_signal);

  void getFourZeroCrossingIntervals(double *filtered_signal, ZeroCrossings &zero_crossings);
  ...
  void getMeanF0(double current_position, double current_f0, int fft_index,
  ...
  inline void getDiffWindow(const double *main_window, int base_time_length,
			    double *diff_window);
  inline void getMainWindow(double current_position, const int *base_index,
  ...
  void mergeF0(const double * const *multi_channel_f0, int *boundary_list,
	       ...
	       double *merged_f0);
  int mergeF0Sub(double *merged_f0, int f0_length, int st1, int ed1,
  ...
  int extend(double **extended_f0, int number_of_sections, int f0_length,
	     int *boundary_list, const double * const *f0_candidates,
	     int number_of_candidates, double allowed_range);
  int extendF0(double *extended_f0, int f0_length, int origin,
