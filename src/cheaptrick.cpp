...
void CheapTrick::compute(const double *x, int x_length, const double *temporal_positions,
...
void CheapTrick::generalBody(const double current_f0, const double current_position,
...
void CheapTrick::getWindowedWaveform(const double current_f0, const double currnet_position,
				     const ForwardRealFFT &forward_real_fft)
{
...
void CheapTrick::getPowerSpectrum(const double f0, const ForwardRealFFT &forward_real_fft)
{
...
void CheapTrick::addInfinitesimalNoise(const double *input_spectrum, double *output_spectrum)
{
  int bin_size = option_.fft_size / 2 + 1;
...
void CheapTrick::smoothingWithRecovery(const double f0,
				       const ForwardRealFFT &forward_real_fft,
				       const InverseRealFFT &inverse_real_fft,
				       double *spectral_envelope)
{
  ...
  for (int i = 0; i <= fft_size / 2; ++i) {
    inverse_real_fft.spectrum[i][0] =
      forward_real_fft.spectrum[i][0] * smoothing_lifter[i] * compensation_lifter[i] / fft_size;
